...
import com.srms.psu.srmspublicapp.service.httpservice.CheckDeviceStatus;
...
//        findViewById(R.id.dummy_button).setOnTouchListener(mDelayHideTouchListener);
        ...
        TextView tvDeviceId = (TextView)findViewById(R.id.fullscreen_content);
        ...
        if(isNetworkAvailable()){
            ...
            canConnectToServer = checkConnectionToServer.canConnectToServer;
        }

        if(canConnectToServer){
            tvDeviceId.setText(android_id);
            CheckDeviceStatus checkDeviceStatus = new CheckDeviceStatus();
            checkDeviceStatus.setContext(context);
            checkDeviceStatus.setWebServiceUrl(webServiceUrl);
            checkDeviceStatus.setDeviceId(android_id);
            try {
                checkDeviceStatus.execute().get();
            } catch (InterruptedException e) {
                e.printStackTrace();
            } catch (ExecutionException e) {
                e.printStackTrace();
            }
            deviceBean = checkDeviceStatus.getDeviceBean();
        }else{
            tvDeviceId.setText("Connection Failed");
            tvPrompt.setText("Unable to connect to the server. Please check your internet connection or you may need to update your app.");
        }

        if(deviceBean != null){
            Intent myIntent = new Intent(RegisterDevice.this, MainActivity.class);
            RegisterDevice.this.startActivity(myIntent);
